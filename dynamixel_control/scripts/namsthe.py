...
def home():
	time.sleep(3)
	y1= np.array([0,0,-.11,0,0,0,0,0,0,0,0,0])
	motor(y1)
	time.sleep(2)
## namste to madam
	#time.sleep(1)
        y3 = np.array([0, 0, -1.2436444286668206, 0.17999695631559007, 1.1391402616488282, -1.4308403775662184, -0.7250993742776569, 1.173747982490342, -0.17996544055454433, -1.1391225148856174, 1.4008939791765995, 0.72019506747458596])
        motor(y3)
	engine = pyttsx3.init()
	rate = engine.getProperty('rate')
	engine.setProperty('rate', rate-15)
	engine.say('Na Mas Thay ')
	engine.runAndWait()
	time.sleep(3)
        y4= np.array([0,0,-.11,0,0,0,0,0,0,0,0,0])
        motor(y4)
	time.sleep(2)
	quit()
...
if __name__ == '__main__':
    rospy.init_node('namasthe', anonymous=True)
    home()
