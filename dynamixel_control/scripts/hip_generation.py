...
if __name__ == '__main__':
    ...
    pub1 = rospy.Publisher('hip_angle_r', Float64, queue_size=10)
    pub2 = rospy.Publisher('knee_angle_r', Float64, queue_size=10)
    pub3 = rospy.Publisher('ankle_angle_r', Float64, queue_size=10)

    pub4 = rospy.Publisher('hip_angle_l', Float64, queue_size=10)
    pub5 = rospy.Publisher('knee_angle_l', Float64, queue_size=10)
    pub6 = rospy.Publisher('ankle_angle_l', Float64, queue_size=10)
...
while (j < 5):
    while (pres_time < total_time):
        ...
        print (hip_val_left[i])
        pub1.publish(hip_val_right[i])
        pub2.publish(knee_val_right[i])
        pub3.publish(ankle_val_right[i])

        pub4.publish(hip_val_left[i])
        pub5.publish(knee_val_left[i])
        pub6.publish(ankle_val_left[i])
